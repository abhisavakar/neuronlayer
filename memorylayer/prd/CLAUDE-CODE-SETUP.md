...
```bash
# Add MemoryLayer as a local stdio server
...
```bash
claude mcp add --transport stdio --env MEMORYLAYER_MAX_TOKENS=8000 memorylayer -- node /path/to/memorylayer/dist/index.js --project .
```

### Option 2: Using JSON Configuration

```bash
claude mcp add-json memorylayer '{
  "type": "stdio",
  "command": "node",
  "args": ["/path/to/memorylayer/dist/index.js", "--project", "."],
  "env": {}
}'
```
...
```bash
# Add a server
claude mcp add --transport stdio memorylayer -- node /path/to/dist/index.js --project .

# List all servers
claude mcp list

# Get server details
claude mcp get memorylayer

# Remove a server
claude mcp remove memorylayer
...
```json
{
  "mcpServers": {
    "memorylayer": {
      "type": "stdio",
      "command": "node",
      "args": ["C:/path/to/memorylayer/dist/index.js", "--project", "."],
      "env": {}
    }
  }
}
...
```json
{
  "mcpServers": {
    "memorylayer": {
      "type": "stdio",
      "command": "node",
      "args": ["${MEMORYLAYER_PATH}/dist/index.js", "--project", "."],
      "env": {
        "MEMORYLAYER_MAX_TOKENS": "${MEMORYLAYER_MAX_TOKENS:-6000}"
      }
    }
  }
}
...
### Method 3: User Configuration (`~/.claude.json`)

For personal servers available across all projects, servers are stored in `~/.claude.json`.

Add with the `--scope user` flag:

```bash
claude mcp add --transport stdio --scope user memorylayer -- node /path/to/memorylayer/dist/index.js --project .
```
...
# Project scope (shareable)
claude mcp add --transport stdio --scope project memorylayer -- node dist/index.js --project .

# User scope (global)
claude mcp add --transport stdio --scope user memorylayer -- node dist/index.js --project .
```
